"use client"
import { useState } from "react"
import Pagination from "@/components/ui/Pagination"
import Table from "@/components/ui/Table"
import TableSearch from "@/components/ui/TableSearch"
import { resultsData } from "@/lib/data"
import { SlidersHorizontal, ArrowUpDown } from "lucide-react"

type Result = {
  id: number
  student: string
  subject: string
  score: number
  type: string
  date: string
}

const columns = [
  { header: "Aluno", accessor: "student" },
  { header: "Disciplina", accessor: "subject" },
  { header: "Nota", accessor: "score" },
  { header: "Tipo", accessor: "type", className: "hidden md:table-cell" },
  { header: "Data", accessor: "date", className: "hidden lg:table-cell" },
]

const getScoreColor = (score: number) => {
  if (score >= 90) return "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400"
  if (score >= 70) return "bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400"
  return "bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400"
}

const renderRow = (item: Result) => (
  <tr key={item.id} className="border-b border-zinc-100 dark:border-zinc-800/50 text-sm hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
    <td className="py-2.5 sm:py-3 px-1.5 sm:px-2">
      <span className="font-bold text-zinc-900 dark:text-zinc-100 text-xs sm:text-sm">{item.student}</span>
    </td>
    <td className="text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm">
      {item.subject}
    </td>
    <td className="py-2.5 sm:py-3 px-1.5 sm:px-2">
      <span className={`px-1.5 sm:px-2 py-0.5 rounded text-[9px] sm:text-[10px] font-bold ${getScoreColor(item.score)}`}>
        {item.score}
      </span>
    </td>
    <td className="hidden md:table-cell">
      <span className="px-1.5 sm:px-2 py-0.5 bg-violet-50 dark:bg-violet-950/40 text-violet-600 dark:text-violet-400 rounded text-[9px] sm:text-[10px] font-bold">
        {item.type}
      </span>
    </td>
    <td className="hidden lg:table-cell text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm">
      {new Date(item.date).toLocaleDateString("pt")}
    </td>
  </tr>
)

const ResultListPage = () => {
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 5

  const totalItems = resultsData.length
  const totalPages = Math.ceil(totalItems / itemsPerPage)

  const paginatedData = resultsData.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  )

  return (
    <div className="m-2 sm:m-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 md:p-6 shadow-sm overflow-hidden">
      <div className="flex flex-col gap-3 sm:gap-4 lg:gap-0 lg:flex-row lg:items-center justify-between mb-4 sm:mb-6">
        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-zinc-900 dark:text-zinc-100">Resultados</h1>
          <p className="text-[11px] sm:text-xs md:text-sm text-zinc-500 dark:text-zinc-400">Acompanhe notas e desempenho dos alunos</p>
        </div>
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
          <div className="flex-1 sm:w-56 md:w-64">
            <TableSearch />
          </div>
          <div className="flex items-center justify-end gap-1.5 sm:gap-2">
            <button className="p-2 sm:p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition active:scale-95">
              <SlidersHorizontal size={16} />
            </button>
            <button className="p-2 sm:p-2.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition active:scale-95">
              <ArrowUpDown size={16} />
            </button>
          </div>
        </div>
      </div>
      <div className="overflow-x-auto -mx-2.5 px-2.5 sm:-mx-4 sm:px-4 md:mx-0 md:px-0">
        <Table columns={columns} renderRow={renderRow} data={paginatedData} />
      </div>
      <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-zinc-100 dark:border-zinc-800">
        <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />
      </div>
    </div>
  )
}

export default ResultListPage
