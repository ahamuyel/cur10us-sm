...
const columns = [
  { header: "Matéria", accessor: "materia" },
  { header: "Turma", accessor: "turma" },
  { header: "Professor", accessor: "professor", className: "hidden md:table-cell" },
...
const renderRow = (item: Exam) => (
  <tr key={item.id} className="border-b border-zinc-100 dark:border-zinc-800/50 text-sm hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors">
    <td className="py-2.5 sm:py-3 px-1.5 sm:px-2">
      <span className="font-bold text-zinc-900 dark:text-zinc-100 text-xs sm:text-sm">{item.materia}</span>
    </td>
    <td className="py-2.5 sm:py-3 px-1.5 sm:px-2">
    ...
    <td className="text-zinc-600 dark:text-zinc-400 text-xs sm:text-sm">
      {new Date(item.data).toLocaleDateString("pt")}
    </td>
  </tr>
)
...
const ExamListPage = () => {
  ...
  const totalItems = examsData.length
