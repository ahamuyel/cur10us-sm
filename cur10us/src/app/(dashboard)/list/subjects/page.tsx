...
const columns = [
  { header: "Disciplina", accessor: "name" },
...
const SubjectListPage = () => {
  ...
  const { data, totalPages, page, search, setSearch, setPage, loading, refetch } = useEntityList<Subject>({ endpoint: "/api/subjects", limit: 5 })
  ...
  return (
    <div className="m-2 sm:m-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 md:p-6 shadow-sm overflow-hidden">
      <div className="flex flex-col gap-3 sm:gap-4 lg:gap-0 lg:flex-row lg:items-center justify-between mb-4 sm:mb-6">
        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-zinc-900 dark:text-zinc-100">Disciplinas</h1>
          <p className="text-[11px] sm:text-xs md:text-sm text-zinc-500 dark:text-zinc-400">Gerencie as disciplinas da escola</p>
        </div>
      ...
      <div className="overflow-x-auto -mx-2.5 px-2.5 sm:-mx-4 sm:px-4 md:mx-0 md:px-0">
        {loading ? (
          ...
          <div className="text-center py-12 text-zinc-400 text-sm">Nenhuma disciplina encontrada</div>
        ) : (
