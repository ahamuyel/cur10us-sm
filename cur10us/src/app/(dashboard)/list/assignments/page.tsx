...
const columns = [
  ...
  { header: "Data de Entrega", accessor: "dueDate" },
...
const AssignmentListPage = () => {
  ...
  const { data, totalPages, page, search, setSearch, setPage, loading, refetch } = useEntityList<Assignment>({ endpoint: "/api/assignments", limit: 5 })
    ...
    <div className="m-2 sm:m-3 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl sm:rounded-2xl p-2.5 sm:p-4 md:p-6 shadow-sm overflow-hidden">
      <div className="flex flex-col gap-3 sm:gap-4 lg:gap-0 lg:flex-row lg:items-center justify-between mb-4 sm:mb-6">
        <div>
          ...
          <p className="text-[11px] sm:text-xs md:text-sm text-zinc-500 dark:text-zinc-400">Gerencie as tarefas e trabalhos</p>
        </div>
      ...
      <FormModal open={!!editItem} onClose={() => setEditItem(null)} title="Editar Tarefa">
        {editItem && (
          <AssignmentForm mode="edit" initialData={editItem} onSuccess={() => { setEditItem(null); refetch() }} onCancel={() => setEditItem(null)} />
        )}
      </FormModal>
